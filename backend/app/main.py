...
def list_jobs() -> List[Dict[str, Any]]:
    return [job.model_dump() for job in store.JOBS.values()]
...
def applications(candidate_id: str) -> List[Dict[str, Any]]:
    return [event.model_dump() for event in store.APPLICATIONS if event.candidate_id == candidate_id]
...
def save_autofill(candidate_id: str, profile: AutofillProfile) -> Dict[str, Any]:
    ...
    store.AUTOFILL_PROFILES[candidate_id] = profile
    return profile.model_dump()


@app.get("/autofill/{candidate_id}")
def get_autofill(candidate_id: str) -> Dict[str, Any]:
    if candidate_id not in store.AUTOFILL_PROFILES:
        candidate = store.get_candidate(candidate_id)
        store.AUTOFILL_PROFILES[candidate_id] = AutofillProfile(candidate_id=candidate_id, legal_name=candidate.name, email=candidate.email, phone=candidate.phone)
