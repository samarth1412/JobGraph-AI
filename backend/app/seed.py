...
def seed() -> None:
    ...
    for title, company, location, description in samples:
        jobs.append(Job(job_id=stable_job_id("demo", title, company, location), source="demo", title=title, company=company, location=location, work_model="remote" if location == "Remote" else "onsite", description=description, apply_url="", required_skills=extract_skills(description)))
