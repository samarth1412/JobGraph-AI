            ...
            <div className="mt-12 grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-start">
              <div>
                ...
                <label className="mt-8 flex min-h-40 cursor-pointer flex-col items-center justify-center rounded-[1.5rem] border border-dashed border-white/15 bg-white/5 p-6 text-center transition hover:bg-white/10">
                  ...
                  <span className="mt-2 text-sm text-zinc-400">PDF, DOCX, or TXT. We use this to create your profile and autofill applications.</span>
                  <input type="file" accept=".pdf,.txt,.docx" className="hidden" onChange={(event) => uploadResume(event.target.files?.[0] || null)} />
                </label>
    ...
    <main className="min-h-screen overflow-hidden bg-[#0f0f10] px-4 py-6 text-white sm:px-6 lg:px-8">
      <section className="mx-auto mb-6 flex w-full max-w-[1400px] flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <nav className="flex flex-wrap gap-2 text-sm font-semibold">
            <Link href="/" className="rounded-full border border-white/10 px-4 py-2 text-zinc-300 hover:bg-white/5">
              Home
            </Link>
            <Link href="/jobs" className="rounded-full border border-white/10 px-4 py-2 text-zinc-300 hover:bg-white/5">
              Job list
            </Link>
            <Link href="/tracker" className="rounded-full border border-white/10 px-4 py-2 text-zinc-300 hover:bg-white/5">
              Tracker
            </Link>
          </nav>
        </div>
      ...
      <section className="mx-auto w-full max-w-[1400px] rounded-[2rem] border border-white/10 bg-[#171717] p-5 shadow-[0_28px_90px_rgba(0,0,0,0.35)] sm:p-6 lg:p-8">
        <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3 text-2xl font-black sm:text-3xl">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-white text-black">
              <Sparkles size={21} />
            </div>
            JobGraph AI
        ...
        <section className="mt-8 rounded-[1.6rem] border border-white/10 bg-[#252525] p-4">
          <div className="grid items-end gap-3 lg:grid-cols-[1fr_1fr_minmax(140px,160px)]">
        ...
        <section className="mt-8 grid gap-8 xl:grid-cols-[1.3fr_0.7fr]">
          <div className="rounded-[2rem] bg-white p-5 text-black md:p-7">
            <h3 className="mb-5 text-3xl font-semibold tracking-tight">Recommended Jobs</h3>
              ...
              {matches.map((match, index) => {
                ...
                return (
                  <article
                    ...
                    className={`grid gap-4 rounded-full px-4 py-3 transition md:grid-cols-[minmax(0,1.6fr)_0.5fr_0.55fr_0.45fr_auto_auto] md:items-center ${
                      selected ? "bg-black text-white shadow-2xl" : "bg-white hover:bg-zinc-50"
                    }`}
                  >
                    <button type="button" className="flex min-w-0 items-center gap-4 text-left" onClick={() => setSelectedId(match.job.job_id)}>
                    ...
                    <span className={`w-fit rounded-full px-4 py-2 text-xs font-semibold capitalize ${selected ? "bg-white/10 text-zinc-300" : "bg-zinc-100 text-zinc-500"}`}>
          ...
          <aside className="space-y-5">
            <section className="rounded-[2rem] border border-white/10 bg-[#292929] p-5">
              ...
              <div className="mt-6 grid grid-cols-2 gap-4">
                ...
                <Detail icon={<Building2 size={18} />} label="Work Mode" value={selectedMatch?.job.work_model || "—"} />
                <Detail
                  icon={<BriefcaseBusiness size={18} />}
                  label="ATS"
                  value={selectedMatch?.job.source ? selectedMatch.job.source.toUpperCase() : "—"}
                />
                <Detail
                  icon={<Sparkles size={18} />}
                  label="Your experience (resume)"
                  value={
                    parsedResume?.experience_years != null && parsedResume.experience_years !== undefined
                      ? `${parsedResume.experience_years} yrs`
                      : "—"
                  }
                />
              </div>
              ...
              {selectedMatch?.missing_skills && selectedMatch.missing_skills.length > 0 && (
                <div className="mt-4">
                  <h4 className="text-lg font-bold">Missing skills</h4>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {selectedMatch.missing_skills.slice(0, 10).map((skill) => (
                      <span key={skill} className="rounded-full border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-sm text-amber-100">
                        {skill}
                      </span>
                    ))}
                  </div>
              ...
              {(selectedMatch?.why_fit?.length || selectedMatch?.why_may_not_fit?.length) && (
                <div className="mt-6 space-y-3 text-sm text-zinc-300">
                  {!!selectedMatch?.why_fit?.length && (
                    <div>
                      <h4 className="text-lg font-bold text-white">Why this job</h4>
                      <ul className="mt-2 list-inside list-disc">
                        {selectedMatch.why_fit.map((line) => (
                          <li key={line}>{line}</li>
                        ))}
                      </ul>
                    </div>
                  )}
                  {!!selectedMatch?.why_may_not_fit?.length && (
                    <div>
                      <h4 className="text-lg font-bold text-white">Why it may not fit</h4>
                      <ul className="mt-2 list-inside list-disc text-zinc-400">
                        {selectedMatch.why_may_not_fit.map((line) => (
                          <li key={line}>{line}</li>
                        ))}
                      </ul>
                    </div>
